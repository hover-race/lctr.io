var globals={};globals.map=null;globals.curMarker=null;globals.lastMinutelyPoint=null;globals.lineByCid={};globals.lastByCid={};var print=function(x){};if(0){print=console.log.bind(console)}function addCircle(){var map=L.map("mapid").setView([51.505,-.09],13);L.tileLayer("https://{s}.tiles.mapbox.com/v4/christianjunk.e3e05ee8/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiY2hyaXN0aWFuanVuayIsImEiOiJkMjIzMzRjNzBlNjc1OWUxYmE0NzBjNzQ3MWNiYTNkMyJ9.q4y4NMEwFYGRZdSEfPBg7A").addTo(map);L.circle([51.5,-.1],800).addTo(map)}function initFb(){var firebaseConfig={apiKey:"AI"+"zaSyCGqggRV1x-"+"cTiV79NKrEHW09cxkSpJodw",authDomain:"c1c1-26bb3.fire"+"baseapp.com",databaseURL:"https://c1c1-26bb3.fi"+"rebaseio.com",projectId:"c1c1-26bb3",storageBucket:"c1c1-26bb3.app"+"spot.com",messagingSenderId:"135213607027",appId:"1:135213607027:we"+"b:28a56032a971394f4be206",measurementId:"G-XB83L0L9Q6"};firebase.initializeApp(firebaseConfig);globals.cid=generateCid()}PREFIX="updates/";function getRef(){return firebase.database().ref(PREFIX+getHash())}function sendUpdate(update){checkCid(update);getRef().push(update)}function checkCid(update){if(!("cid"in update)){update.cid=globals.cid}}function listenForUpdates(){print("listenForUpdates");var ref=getRef();ref.on("child_added",handlePointUpdate)}function handlePointUpdate(update){receiveNewPoint(update.val())}function initMain(){setHash();initGeoposition();initMap();initFb();listenForUpdates()}function initGeoposition(){if(geoPosition.init()){requestPosition();print("schedule");setInterval(requestPosition,GEO_UPDATE_INTERVAL_MS)}else{alert("You cannot use Geolocation in this device")}}const GEO_UPDATE_INTERVAL_SEC=5;const GEO_UPDATE_INTERVAL_MS=GEO_UPDATE_INTERVAL_SEC*1e3;function geo_success_callback(location){print(location);var update={ts:getTs(),lat:location.coords.latitude,lng:location.coords.longitude};print(update);sendUpdate(update)}function error_callback(p){alert(p.message)}function requestPosition(){print("requestPosition");geoPosition.getCurrentPosition(geo_success_callback,error_callback,{enableHighAccuracy:true})}function getTs(){return Math.floor(Date.now()/1e3)}function setHash(){const HASH_LENGTH=4;if(location.hash==null||location.hash==""){location.hash=generateAlphaHash(HASH_LENGTH)}}function getHash(){if(location.hash.length>1&&location.hash.startsWith("#")){return location.hash.substring(1)}return location.hash}function generateAlphaHash(length){const charset="abcdefghijklmnopqrstuvwxyz";values=new Uint32Array(length);window.crypto.getRandomValues(values);var result="";for(var i=0;i<length;i++){result+=charset[values[i]%charset.length]}return result}function generateCid(){const CID_LENGTH=8;return generateAlphaHash(CID_LENGTH)}function main(){initMap();showBusRoute()}function initMap(){var map=L.map("mapid");globals.map=map;globals.line=L.polyline([]).addTo(globals.map);centerCoords(map,37.77,-122.45);addMapboxLayer(map)}function centerCoords(map,lat,lng){map.setView([lat,lng],13)}function addMapboxLayer(map){L.tileLayer("https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",{attribution:'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors, <a href="https://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="https://www.mapbox.com/">Mapbox</a>',maxZoom:18,id:"mapbox/streets-v11",accessToken:"pk.eyJ1IjoiYm9ocm5rcyIsImEiOiJjazNzbzJnbTkwOTZ2M2lsM2xqaHBqcGoxIn0.IF5EkYs3RLcwZzx8vRqRrw "}).addTo(map)}function addOSMLayer(map){L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(map)}function showJsonRoute(map,geojson){var lineStyle={color:"#ff7800",weight:3,opacity:.85,dashArray:"12 7"};L.geoJson(geojson,{style:lineStyle}).addTo(map)}function receiveNewPoint(point){var ll=L.latLng(point.lat,point.lng);getLine(point.cid).addLatLng(ll);print(point);globals.lastByCid[point.cid]=ll;checkPointInBounds(point);checkMinutelyMarker(point)}function checkPointInBounds(newLatLng){var viewportBounds=globals.map.getBounds();var lastPoints=Object.values(globals.lastByCid);if(lastPoints.some(pt=>viewportBounds.contains(pt))){return}globals.map.flyTo(newLatLng)}function getLine(cid){if(cid in globals.lineByCid){return globals.lineByCid[cid]}var lineColor=getNextColor();var line=L.polyline([],{color:lineColor}).addTo(globals.map);globals.lineByCid[cid]=line;return line}function getLastPoint(cid){var line=getLine(cid);var latLngs=line.getLatLngs();if(latLngs.length==0){return null}return latLngs.slice(-1)[0]}function getLineColor(cid){var line=getLine(cid);return line.options.color}function getNextColor(){const colors=["forestgreen","royalblue","orange","turquoise","darkred","limegreen","blueviolet","chocolate","salmon","olive","gray"];var numLines=Object.keys(globals.lineByCid).length;var i=numLines%colors.length;return colors[i]}function makeMarker(lat,lng){var cssIcon=L.divIcon({className:"css-icon",html:'<div class="gps_ring"></div>',iconSize:[22,22]});return L.marker([lat,lng],{icon:cssIcon})}function addMinutelyMarker(newPoint){var markerColor=getLineColor(newPoint.cid);L.circle([newPoint.lat,newPoint.lng],100,{color:markerColor}).addTo(globals.map);globals.lastMinutelyPoint=newPoint}function checkMinutelyMarker(newPoint){var MARKER_EVERY_SECONDS=120;if(globals.lastMinutelyPoint==null||newPoint.ts-globals.lastMinutelyPoint.ts>MARKER_EVERY_SECONDS){addMinutelyMarker(newPoint)}}function moveCurMarker(points){if(points.length==0){return}var curPoint=points[0];if(globals.curMarker==null){globals.curMarker=makeMarker(curPoint.lat,curPoint.lng).addTo(globals.map)}var ll=L.latLng(curPoint.lat,curPoint.lng);globals.curMarker.setLatLng(ll)}
/* @preserve
